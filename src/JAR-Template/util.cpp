...
/**
 * @brief Restricts the maximum acceleration or deceleration
 *
 * @param target The target value that you want ot reach
 * @param current The current value that you're at
 * @param maxChange The maximum amount the current value can change by
...
{
  float ratio = max(fabs(drive_output + heading_output), fabs(drive_output - heading_output)) / 12.0;
  if (ratio > 1)
  {
